    ...
    def process(self):
        ...
        for ent, rend in self.world.get_component(Drawable):
            if rend.form.FormType == Forms.TEXT:
                ...
                if self.world.component_for_entity(ent, VoidComponent).void_type == VoidType.BOID and self.world.has_component(ent, Velocity):
                    vect = self.world.component_for_entity(ent, Velocity).pos
                    item = pygame.Rect(vect[0], vect[1], rend.w, rend.h)
                ...
                if  self.world.component_for_entity(ent, VoidComponent).void_type == VoidType.HOIK and self.world.has_component(ent, Velocity):
